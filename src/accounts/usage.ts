...
function oauthHeaders(access: string): Record<string, string> {
  return {
 ...
 */
export function parseUsageHeaders(headers: Headers): Usage | null {
...
export function isLimitResponse(response: Response, body: string): boolean {
  if (response.status === 429) return true
  if (headersSayRejected(response.headers)) return true
...
function headersSayRejected(headers: Headers): boolean {
...
export function resetFromResponse(response: Response): number {
  return (
    num(response.headers.get('anthropic-ratelimit-unified-5h-reset')) ??
 ...
 */
export async function probeUsage(access: string): Promise<Usage | null> {
  try {
    ...
    const seconds = (iso: string | null | undefined): number =>
...
export async function fetchProfile(access: string): Promise<Profile | null> {
  try {
    const response = await fetch(PROFILE_URL, { headers: oauthHeaders(access) })
    if (!response.ok) return null
    const json = (await response.json()) as {
      account?: { uuid?: string; email?: string }
      organization?: { name?: string; rate_limit_tier?: string }
    }
    if (!json.account?.uuid || !json.account.email) return null
    ...
    return {
      ...
      tier:
        json.organization?.rate_limit_tier?.replace(/^default_claude_/, '') ??
        null,
    }
  } catch {
    return null
